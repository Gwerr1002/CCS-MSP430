...
{
    switch(hex[0]){
    case 'x': //SI SE RECIBE UN CARÁCTER X DE LA APP CAMBIA LCD A CONTINUO
        positionLCD(0);
        WRITE("Continuo");
        PERIODO = 0;
        break;
    case 'y': //SI SE RECIBE UN CARÁCTER Y DE LA APP CAMBIA LCD A CONTINUO
        positionLCD(0);
        WRITE("Discreto");
        PERIODO = 0;
        break;
    default: //EN CUALQUIER OTRO CASO PROCEDE A CONVERTIR EL TEXTO RECIBIDO (HEXADECIMAL) A DECIMAL
        PERIODO = (char_hex(hex[0]) << 8) | (char_hex(hex[1]) << 4) | char_hex(hex[2]); //CONCATENA EL VALOR BINARIO DE CADA DÍGITO HEX
        break;
    }
}
...
{
    int dec;
    if((u >= 48) && (u <= 57)) dec = u-48; //SI RECIBE UN CARÁCTER DEL 0-9 (48-57) RESTA 48 QUEDANDO EL DECIMAL
    if((u >= 65) && (u <= 70)) dec = u-55; //SI RECIBE UNA LETRA DE A-F (65-70) RESTA 55 DE TAL FORMA QUE SI SE MANDA A DE COMO RESLTADO 10 DECIMAL
    return dec;
}

void FREC(void)
{
    //CONVERSION DE DECIMAL 4 DÍGITOS A CADENA DE CARACTERES
    unsigned char F[6];
    unsigned int frec;
    if(PERIODO != 0){ //PARA EVITAR DIVIDIONES ENTRE 0
        frec = 1000000/(PERIODO); //CONVERSION PWM - HZ
        F[0] = '0' + (frec)/1000; //UNIDADES DE MILLAR - CHAR
        F[1] = '0' + (frec % 1000)/100; //CENTENAS - CHAR
        F[2] = '0' + (frec % 100)/10; //DECENAS- CHAR
        F[3] = '0' + (frec % 10); //UNIDADES - CHAR
        F[4] = 'H';
        F[5] = 'z';
        positionLCD(10);
        WRITE(F);
...
{
    /*
     * EN GENERAL SELLECCION DE COLOR Y MUESTRA EN LCD, DE ACUERDO AL PWM ACTUAL DE ACUERDO A LA DISTANCIA ENTRE NOTA Y NOTA
     * TAMBIEN MUESTRA EN EL LCD LA NOTA ACTUAL
     */
    if(PERIODO == 0)
    {
        R = 33; G = 33; B = 34;
